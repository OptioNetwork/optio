...
// setupWithOwner returns the keeper, msg server, unwrapped context, and the owner address from params.
...
func TestIssueLicense(t *testing.T) {
	...
	// Create license type with max_supply=10
	...
	tests := []struct {
		...
		expErr    bool
		expErrMsg string
		expCount  int // expected number of IDs returned
	}{
		{
			name: "no permission",
			input: &types.MsgIssueLicense{
		...
		{
			name: "type not found (fails at permission check)",
			input: &types.MsgIssueLicense{
				Issuer:        issuer,
				LicenseTypeId: "missing",
				Holder:        holder,
				StartDate:     "2026-01-01",
			},
			expErr:    true,
		...
		{
			name: "valid single (count=0 defaults to 1)",
			input: &types.MsgIssueLicense{
				Issuer:        issuer,
				LicenseTypeId: "node",
				Holder:        holder,
		...
		},
		{
			...
			input: &types.MsgIssueLicense{
				...
				Holder:        holder,
				StartDate:     "2026-01-01",
				Count:         10, // already issued 4, would go to 14
			},
			expErr:    true,
	...
	require.Equal(t, math.NewInt(4), lt.IssuedCount) // 1 + 3
}
...
func TestBatchIssueLicense(t *testing.T) {
	...
	holder3 := sample.AccAddress()

	// Create license type with max_supply=5
	_, err := ms.CreateLicenseType(ctx, &types.MsgCreateLicenseType{
		Owner:     owner,
		Id:        "batch",
		MaxSupply: math.NewInt(5),
	})
	require.NoError(t, err)
	...
	tests := []struct {
		...
		},
		{
			...
			input: &types.MsgBatchIssueLicense{
				...
				LicenseTypeId: "batch",
				Entries: []types.BatchIssueLicenseEntry{
					{Holder: holder1, StartDate: "2026-01-01"},
				},
			},
			expErr:    true,
			expErrMsg: "does not have issue permission",
		...
		},
		{
			...
			input: &types.MsgBatchIssueLicense{
				...
				LicenseTypeId: "batch",
				Entries: []types.BatchIssueLicenseEntry{
					{Holder: holder1, StartDate: "bad-date"},
				},
			},
			expErr:    true,
			expErrMsg: "YYYY-MM-DD",
		},
		{
			...
			input: &types.MsgBatchIssueLicense{
				...
				LicenseTypeId: "batch",
				Entries: []types.BatchIssueLicenseEntry{
					{Holder: holder1, StartDate: "2026-01-01", EndDate: "2027-01-01"},
					{Holder: holder2, StartDate: "2026-02-01"},
					{Holder: holder3, StartDate: "2026-03-01"},
				},
			},
			expErr:   false,
			expCount: 3,
		},
		{
			...
			input: &types.MsgBatchIssueLicense{
				...
				Entries: []types.BatchIssueLicenseEntry{
					{Holder: holder1, StartDate: "2026-01-01"},
					{Holder: holder2, StartDate: "2026-01-01"},
					{Holder: holder3, StartDate: "2026-01-01"},
				},
			},
			expErr:    true,
			expErrMsg: "exceed max supply",
...
func TestRevokeLicense(t *testing.T) {
	...
	// Setup: create type, grant issue+revoke, issue a license
	_, err := ms.CreateLicenseType(ctx, &types.MsgCreateLicenseType{
	...
	tests := []struct {
	...
	}{
		{
			name: "not found",
			input: &types.MsgRevokeLicense{
				Revoker: revoker, LicenseTypeId: "rev", Id: 999,
			},
			expErr:    true,
			expErrMsg: "not found",
		},
		{
			name: "no permission",
			input: &types.MsgRevokeLicense{
				Revoker: sample.AccAddress(), LicenseTypeId: "rev", Id: licenseID,
			},
			expErr:    true,
		...
		},
		{
			name: "valid",
			input: &types.MsgRevokeLicense{
				Revoker: revoker, LicenseTypeId: "rev", Id: licenseID,
			},
			expErr: false,
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expErr {
				...
				_, found, err := k.GetLicense(ctx, "rev", licenseID)
				require.NoError(t, err)
				require.False(t, found)
				// Verify issued count decremented
...
func TestUpdateLicense(t *testing.T) {
	...
	// Setup: create type, grant issue+update, issue a license
	_, err := ms.CreateLicenseType(ctx, &types.MsgCreateLicenseType{
	...
	tests := []struct {
	...
	}{
		{
			name: "not found",
			input: &types.MsgUpdateLicense{
				Updater: updater, LicenseTypeId: "upd", Id: 999, Status: "revoked",
			},
			expErr:    true,
			expErrMsg: "not found",
		},
		{
			name: "no permission",
			input: &types.MsgUpdateLicense{
				Updater: sample.AccAddress(), LicenseTypeId: "upd", Id: licenseID, Status: "revoked",
			},
			expErr:    true,
		...
		},
		{
			name: "invalid status",
			input: &types.MsgUpdateLicense{
				Updater: updater, LicenseTypeId: "upd", Id: licenseID, Status: "suspended",
			},
			expErr:    true,
			expErrMsg: "invalid status",
		},
		{
			name: "valid - set revoked",
			input: &types.MsgUpdateLicense{
				Updater: updater, LicenseTypeId: "upd", Id: licenseID, Status: "revoked",
			},
			expErr: false,
		},
	}
...
func TestTransferLicense(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			input: &types.MsgTransferLicense{
				Holder: holder, LicenseTypeId: "xfer", Id: xferID, Recipient: "bad",
			},
			expErr:    true,
		...
		},
		{
			...
			input: &types.MsgTransferLicense{
				Holder: holder, LicenseTypeId: "xfer", Id: xferID, Recipient: holder,
			},
			expErr:    true,
		...
		},
		{
			name: "not found",
			input: &types.MsgTransferLicense{
				Holder: holder, LicenseTypeId: "xfer", Id: 999, Recipient: recipient,
			},
			expErr:    true,
			expErrMsg: "not found",
		},
		{
			name: "not holder",
			input: &types.MsgTransferLicense{
				Holder: sample.AccAddress(), LicenseTypeId: "xfer", Id: xferID, Recipient: recipient,
			},
			expErr:    true,
		...
		},
		{
			...
			input: &types.MsgTransferLicense{
				Holder: holder, LicenseTypeId: "noxfer", Id: noxferID, Recipient: recipient,
			},
			expErr:    true,
		...
		},
		{
			name: "valid",
			input: &types.MsgTransferLicense{
				Holder: holder, LicenseTypeId: "xfer", Id: xferID, Recipient: recipient,
			},
			expErr: false,
		},
	}
...
func TestUpdateLicenseType(t *testing.T) {
	...
	// Create license type and issue some licenses
	_, err := ms.CreateLicenseType(ctx, &types.MsgCreateLicenseType{
	...
	tests := []struct {
	...
	}{
		{
			name: "non-owner",
			input: &types.MsgUpdateLicenseType{
				Owner: sample.AccAddress(), Id: "lt1", Transferrable: true, MaxSupply: math.NewInt(200),
			},
			expErr:    true,
			expErrMsg: "not the module owner",
		},
		{
			name: "not found",
			input: &types.MsgUpdateLicenseType{
				Owner: owner, Id: "missing", Transferrable: true, MaxSupply: math.NewInt(200),
			},
			expErr:    true,
			expErrMsg: "not found",
		},
		{
			...
			input: &types.MsgUpdateLicenseType{
				Owner: owner, Id: "lt1", Transferrable: true, MaxSupply: math.NewInt(3), // 5 issued
			},
			expErr:    true,
		...
		},
		{
			name: "valid update",
			input: &types.MsgUpdateLicenseType{
				Owner: owner, Id: "lt1", Transferrable: true, MaxSupply: math.NewInt(200),
			},
			expErr: false,
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expErr {
				...
				require.NoError(t, err)
				lt, found, _ := k.GetLicenseType(ctx, "lt1")
				require.True(t, found)
				require.True(t, lt.Transferrable)
				require.Equal(t, math.NewInt(200), lt.MaxSupply)
			}
		})
	}
}
